...
<tr>
 ...
 <td>
  ...
  <td>
  <?php  
  if($proposal->status == 0)echo '<span  class="label label-warning">Pending</span> on '.$proposal->status_on; 
  ...
  <tr><td><strong>Proposal Updated By :</strong></td><td><?php echo $proposal->status_by; ?></td></tr>
  
   <tr>
  <td>
  <strong>Assigned Committee Member : </strong>
  </td>
  <td>
  <?php 
  if($assigned_cm != NULL)echo $assigned_cm->first_name.' '.$assigned_cm->last_name; 
  else echo '-';
  ?> 
  </td>
  </tr>
...
<div class="block">
                ...
                <form id="form-change-status" action="<?php echo site_url('proposals/change_cm/'.$proposal->proposal_id); ?>" method="post" class="form-horizontal form-bordered" >
