...
$this->load->helper('email');
	...
	if ($this->input->post('btn_cancel'))redirect('sessions/signup');
	   ...
	   $data_array['raw_ps'] = $this->input->post('password');
	   ...
	   $data_array['fax'] = '';
       if($data_array['salutation'] == 'Other')$data_array['custom_salutation'] = $this->input->post('custom_salutation',true);
	   else $data_array['custom_salutation'] = '';	   	   
	   $data_array['proposer'] = 1;
	   $id = $this->mdl_sessions->save(NULL, $data_array);
	   if($id > 0){
	   $salutation = $data_array['salutation'];
	   if($data_array['salutation'] == 'Other') $salutation = $data_array['custom_salutation']; 
	   
	   $name = trim($salutation.' ' .$_POST['first_name']." ".$_POST['last_name']);
