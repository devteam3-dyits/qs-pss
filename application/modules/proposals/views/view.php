      ...
      <td>
        <?php 
          if($proposal->video_url){ ?>
            <span style="font-size: 12px; color: green;">Your video is uploaded successfully.</span>
        <?php  }else{ ?>
            <span style="font-size: 12px; color: red;">Your video is not uploaded.</span>
        <?php  }
        
        ?>
      </td>
    </tr>
    <tr <?php if ($proposal->status == 1) echo 'style="display:none"'; ?>>
      <td><strong><?php if ($proposal->status == 3) echo 'Reason for rejecting';
                  elseif ($proposal->status == 2) echo 'Reason for provisionally accepting'; ?> </td>
      <td><?php echo $proposal->reason; ?></td>
    </tr>
    <tr>
      <td>This proposal updated by</td>
      <td><?php echo $proposal->status_by; ?></strong></td>
    </tr>
  </table>
</div>
<div class="block">
