...
class Controller extends MX_Controller {
    ...
    public function __construct()
    {
       ...
       $this->load->library('session');		
		...
		if($this->session->userdata('user_id') <= 0   || $this->session->userdata('user_type') != 'proposer'){
		redirect('/sessions/login');
		
		
		  }
		  if( $this->session->userdata('job_title') == 'No')
		  $this->session->set_flashdata('alert_error','Almost there! Please complete your profile to continue.');
